...
PacketHandler.prototype.message_onKeyQ = function (message) {
    ...
    if (dt < this.darkServer.config.ejectCooldown) {
        Logger.info("Q press too quick");
        return;
    }
    this.lastQTick = tick;
    if (this.socket.playerTracker.minionControl && !this.darkServer.config.disableQ) {
...
PacketHandler.prototype.sendPacket = function(packet) {
    ...
    if (!packet || socket.isConnected == null || socket.playerTracker.isMi) 
        return;
    if (socket.readyState == this.darkServer.WebSocket.OPEN) {
        var buffer = packet.build(this.protocol);
        if (buffer) socket.send(buffer, { binary: true });
    } else {
        socket.readyState = this.darkServer.WebSocket.CLOSED;
        socket.emit('close');
    }
};
