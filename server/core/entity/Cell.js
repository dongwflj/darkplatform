function Cell(darkServer, owner, position, size) {
    ...
    if (this.darkServer) {
        this.tickOfBirth = this.darkServer.tickCounter;
        this.nodeId = this.darkServer.lastNodeId++ >> 0;
...
Cell.prototype.getAge = function () {
    ...
    return Math.max(0, this.darkServer.tickCounter - this.tickOfBirth);
};
...
Cell.prototype.setBoost = function (distance, angle) {
    ...
    this.isMoving = true;
    if (!this.owner) {
        var index = this.darkServer.movingNodes.indexOf(this);
        if (index < 0)
            this.darkServer.movingNodes.push(this);
    }
};
...
Cell.prototype.onAdd = function (darkServer) { };
Cell.prototype.onRemove = function (darkServer) { };
