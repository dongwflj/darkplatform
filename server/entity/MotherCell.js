...
MotherCell.prototype.canEat = function (cell) {
    var maxMass = this.darkServer.config.motherCellMaxMass;
...
MotherCell.prototype.onUpdate = function () {
    ...
    var maxFood = this.darkServer.config.foodMaxAmount;
    if (this.darkServer.nodesFood.length >= maxFood) {
        return;
    }
    var size1 = this._size;
    var size2 = this.darkServer.config.foodMinSize;
    for (var i = 0; i < this.motherCellSpawnAmount; i++) {
        ...
        // Spawn food
        var food = new Food(this.darkServer, null, pos, size2);
        food.setColor(this.darkServer.getRandomColor());
        this.darkServer.addNode(food);
        
        // Eject to random distance
        food.setBoost(32 + 32 * Math.random(), angle);
        
        if (this.darkServer.nodesFood.length >= maxFood || size1 <= this.motherCellMinSize) {
            break;
        }
    }
    this.darkServer.updateNodeQuad(this);
};
