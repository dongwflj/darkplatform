...
var playerCommands = {
    ...
    mass: function (args) {
        ...
        if (isNaN(id)) {
            ...
            for (var i in this.darkServer.clients) {
                var client = this.darkServer.clients[i].playerTracker;
                if (client.pID == id) {
                    ...
                    this.darkServer.sendChatMessage(null, client, text);
                    break;
                }
            }
        }
    ...
    minion: function(args) {
        ...
        if (isNaN(id)) {
            ...
            // Add minions
                ...
                for (var i = 0; i < add; i++) {
                    this.darkServer.bots.addMinion(player);
                }
            ...
            for (var i in this.darkServer.clients) {
                ...
                if (client.pID == id) {
                    ...
                    // Add minions
                        ...
                        if (isNaN(add)) add = 1;
                        for (var i = 0; i < add; i++) {
                            this.darkServer.bots.addMinion(client);
                        }
    ...
    status: function(args) {
        ...
        var humans = 0,
            bots = 0;
        for (var i = 0; i < this.darkServer.clients.length; i++) {
            if ('_socket' in this.darkServer.clients[i]) {
                humans++;
            } else {
                bots++;
            }
        ...
        this.writeLine("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
        this.writeLine("Connected players: " + this.darkServer.clients.length + "/" + this.darkServer.config.serverMaxConnections);
        ...
        this.writeLine("Current game mode: " + this.darkServer.darkMode.name);
        this.writeLine("Current update time: " + this.darkServer.updateTimeAvg.toFixed(3) + " [ms]  (" + ini.getLagMessage(this.darkServer.updateTimeAvg) + ")");
        this.writeLine("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
    },
    login: function (args) {
        var password = args[1] + "";
