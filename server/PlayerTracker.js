...
/*
    ...
    Logger.info("player maxx:" + maxx + " minx:" + minx);
    Logger.info("player maxy:" + maxy + " miny:" + miny);
    var dist = vec.sqDist();
    Logger.info("player dist:" + dist);
    var distScale = (this.darkServer.config.borderWidth - dist)/this.darkServer.config.borderWidth;   
    Logger.info("origin scale:" + scale);
    Logger.info("dist scale:" + distScale);
    var scale = Math.min(scale, distScale); */
    return scale;
};
...
PlayerTracker.prototype.joinGame = function(name, skin) {
    ...
    if (!this.isMi && this.socket.isConnected != null) {
        ...
        if (this.darkServer.config.serverScrambleLevel < 2) {
            ...
            packetHandler.sendPacket(new Packet.SetBorder(this, this.darkServer.border));
        }
...
PlayerTracker.prototype.checkConnection = function() {
    ...
    if (!this.socket.isConnected) {
        ...
        var pt = this.darkServer.config.playerDisconnectTime;
        var dt = (this.darkServer.stepDateTime - this.socket.closeTime) / 1e3;
        if (pt && (!this.cells.length || dt >= pt)) {
            // Remove all client cells
            while (this.cells.length) this.darkServer.removeNode(this.cells[0]);
        }
        this.cells = [];
        this.isRemoved = true;
        this.mouse = null;
...
/**    var scale = Math.max(this.getScale(), this.darkServer.config.serverMinScale);
    ...
    var halfWidth = (this.darkServer.config.serverViewBaseX + 100) / scale / 2;
    var halfHeight = (this.darkServer.config.serverViewBaseY + 100) / scale / 2;
...
PlayerTracker.prototype.sendUpdate = function() {
    ...
    if (++this.tickLeaderboard > 25) {
        ...
        if (this.darkServer.leaderboardType >= 0)
            packetHandler.sendPacket(new Packet.UpdateLeaderboard(this, this.darkServer.leaderboard, this.darkServer.leaderboardType));
    }
};
...
PlayerTracker.prototype.updateSpecView = function(len) {
    ...
    if (!this.spectate || len) {
        ...
        if (this.freeRoam || this.getSpecTarget() == null) {
            ...
            var scale = this.darkServer.config.serverSpectatorScale;
            if (this.centerPos.x < this.darkServer.border.minx + 6000) {
                this.centerPos.x = this.darkServer.border.minx + 6000;
            }
            else if (this.centerPos.x > this.darkServer.border.maxx - 6000) {
                this.centerPos.x = this.darkServer.border.maxx - 6000;
            }
            if (this.centerPos.y < this.darkServer.border.miny + 3000) {
                this.centerPos.y = this.darkServer.border.miny + 3000;
            }
            else if (this.centerPos.y > this.darkServer.border.maxy - 3000) {
                this.centerPos.y = this.darkServer.border.maxy - 3000;
            }
...
PlayerTracker.prototype.pressSpace = function() {
    if (this.spectate) {
        ...
        if (this.darkServer.tickCounter - this.lastKeypressTick < 40)
            return;
        this.lastKeypressTick = this.darkServer.tickCounter;
...
PlayerTracker.prototype.pressW = function() {
    if (this.spectate || !this.darkServer.run) return;
...
PlayerTracker.prototype.setCenterPos = function(p) {
    p.x = Math.max(p.x, this.darkServer.border.minx);
    p.y = Math.max(p.y, this.darkServer.border.miny);
    p.x = Math.min(p.x, this.darkServer.border.maxx);
    p.y = Math.min(p.y, this.darkServer.border.maxy);
    this.centerPos = p;
};
